...
- **fal MCP is integrated, not built:** The `.claude/skills/weave-broll/SKILL.md` skill generates footage through the **hosted** fal MCP (`https://mcp.fal.ai/mcp`) — we do not build, vendor, or maintain an MCP server here. It is **opt-in and user-keyed** (the user registers it via `claude mcp add` with their own fal API key and approves each paid run); never register it or trigger a paid job without explicit consent. fal-generated clips are tool output → they follow the output-folder policy above (`outputs/`), never committed.
